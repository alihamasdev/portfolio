...
import { SVGMotionProps, useAnimation, type LegacyAnimationControls, type Variants } from "motion/react";
...
interface IconProps<T>
	extends DefaultIconProps<T>, Omit<SVGMotionProps<SVGSVGElement>, "animate" | "onAnimationStart" | "onAnimationEnd"> {
	size?: number;
}
...
function useAnimateIconContext() {
	...
	if (!context) return { controls: undefined, animation: "default", loop: false, loopDelay: 0 };
	return context;
}
...
function AnimateIcon({
	...
	return (
		<AnimateIconContext.Provider value={{ controls, animation: currentAnimation.current, loop, loopDelay }}>
			{cloned}
...
function IconWrapper<T extends string>({
	...
	if (context) {
		...
		return (
			<AnimateIconContext.Provider
				value={{ controls, animation: animationToUse, loop: loopToUse, loopDelay: loopDelayToUse }}
			>
				<IconComponent
					size={size}
			...
			if (hasNestedKeys) {
				for (const prop in transition) {
					...
					if (typeof subTrans === "object" && subTrans !== null) {
						transition[prop] = { ...subTrans, repeat: Infinity, repeatType: "loop", repeatDelay: loopDelay };
					}
				}
			} else {
				state.animate.transition = { ...transition, repeat: Infinity, repeatType: "loop", repeatDelay: loopDelay };
			}
		}
	}
